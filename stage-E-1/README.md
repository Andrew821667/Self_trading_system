...
## Инвентарь заморожен 06.09.2026 (тег `e1-inventory-frozen`)

Что заморожено:

| | |
|---|---|
| событий в `inventory/events.jsonl` | **296** |
| документов | **854** |
| допущено к классификации | **172** |
| выборка с котировкой до объявления | **39** |
| годы | 2019–2025 |

Инвентарь воспроизводим: пересборка тремя командами конвейера выше даёт
`events.jsonl` с тем же SHA-256, а повторный прогон не меняет ни одного
`.meta.json`. Проверено перед постановкой тега.

**Покрытие полное по тому, что влияет на вердикт, и неполное в целом** —
формулировка намеренная, см. `coverage_report.md`. Из ленты «Интерфакса»
собраны тексты всех сообщений торгуемых эмитентов; тексты примерно 350
сообщений непубличных и делистингованных компаний не собраны. Они
пополнили бы инвентарь, но ни одного наблюдения для статистики дать не
могут: их акции негде купить, спреда нет.

После тега `events.jsonl` и сохранённые документы задним числом не
редактируются. Ошибка — это новая запись с объяснением в коммите, а не
переписывание истории; существенное пополнение — версия инвентаря, а не
правка замороженной.
